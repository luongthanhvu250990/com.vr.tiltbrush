...
Shader "Brush/StandardDoubleSided" {
    Properties {
      ...
      _Cutoff ("Alpha cutoff", Range(0,1)) = 0.5
          ...
          // -------- FAKE LIGHT (HDR) --------
      [HDR]_FakeLightColor("Fake Light Color", Color) = (0.4,0.6,1,1)
      _FakeLightPos("Fake Light Position", Vector) = (0,2,0,1)
      _FakeLightRange("Fake Light Range", Float) = 5
    ...
    float4 _FakeLightPos;
    float _FakeLightRange;
    float _FakeLightIntensity;

    half3 ComputeFakeLight(float3 worldPos)
      {
          float d = distance(worldPos, _FakeLightPos.xyz);
          float atten = saturate(1.0 - d / _FakeLightRange);
          return _FakeLightColor.rgb * atten * _FakeLightIntensity;
      }
    ENDCG
  ...
  SubShader {
    ...
    #include "../../../Shaders/Include/Brush.cginc"
      ...
      void surf(Input IN, inout SurfaceOutputStandardSpecular o) {
        ...
        // FAKE LIGHT (HDR, no dynamic light)
        o.Emission += ComputeFakeLight(IN.worldPos);
      }
      ENDCG
    }
    ...
    Pass {
      ...
      CGPROGRAM
        ...
        fixed4 frag(v2f i, fixed vface : VFACE) : SV_Target {
          fixed4 col = i.color;
          half3 n = normalize(i.worldNormal * vface);

          fixed ndotl = saturate(dot(n, normalize(_WorldSpaceLightPos0.xyz)));
          fixed3 lighting = ndotl * _LightColor0;
          lighting += ShadeSH9(half4(n,1));

          // FAKE LIGHT
          lighting += ComputeFakeLight(i.worldPos);
